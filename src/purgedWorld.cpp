...
PurgedWorld::PurgedWorld(double xs, double xe, double ys, double ye, double st,
                         Obstacle *o)
...
void PurgedWorld::Ti(double x, double y, StarTiInfo &ti, double &resX,
                     double &resY) {
  if (ti.x0 == x && ti.y0 == y) {
    resX = x;
    resY = y;
...
double PurgedWorld::PotentialValue(double x, double y, double kappa) {
  ...
  double betaP = theParentBeta + (2 * gap - theNewBeta) +
                 sqrt(theParentBeta * theParentBeta +
                      (2 * gap - theNewBeta) * (2 * gap - theNewBeta));
  ...
  double tempX, tempY;
  StarTiInfo T0 = {virtObs->centerX, virtObs->centerY, virtObs->Rho(x, y),
                   theNewBeta};
