...
StarWorld::StarWorld(double xs, double xe, double ys, double ye, double st,
                     Obstacle *o)
    : World(xs, xe, ys, ye, st, o) {}

void StarWorld::Ti(double x, double y, StarTiInfo &ti, double &resX,
                   double &resY, bool isMain, double maxMain) {
  if (ti.x0 == x && ti.y0 == y) {
    resX = x;
    resY = y;
    return;
  ...
  double v;
  if (isMain)
    v = (1 - ti.betaSon / maxMain) * ti.rhoParent / norm;
  else
    v = (1 + ti.betaSon) * ti.rhoParent / norm;
...
double StarWorld::PotentialValue(double x, double y, double kappa) {
  ...
  for (int i = 0; i < length; i++) {
    ...
    s.push_back(gamma * ommitProduct /
                (gamma * ommitProduct + kappa * betas[i]));
    theS -= s[i];
  }

  double tempX, tempY;
  StarTiInfo T0 = {mainObs->centerX, mainObs->centerY,
                   mainObs->parentObs->Rho(x, y), -theBeta};
  ...
  for (int i = 0; i < length; i++) {
    StarTiInfo T = {obsArray[i]->centerX, obsArray[i]->centerY,
                    obsArray[i]->parentObs->Rho(x, y), betas[i]};
